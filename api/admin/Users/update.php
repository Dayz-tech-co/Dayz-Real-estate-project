...
if (getenv('REQUEST_METHOD') === $api_method) {
    try {
        ...
        $update = $db_call_class->updateRows("users", ["status" => $new_status], [[
            ['column' => 'id', 'operator' => '=', 'value' => $user_id]
        ]]);
        ...
        if ($update > 0) {
            ...
            $api_status_code_class_call->respondOK(
                ...
                API_User_Response::$userstatusupdated
            );
        } else {
            $api_status_code_class_call->respondInternalError($update, API_User_Response::$failtoupdateuser);
        }
