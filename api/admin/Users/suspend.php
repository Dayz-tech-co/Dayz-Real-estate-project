...
if (getenv('REQUEST_METHOD') === $api_method) {
    try {
        ...
        $update = $db_call_class->updateRows("users", ["status" => "suspended"], [[
            ['column' => 'id', 'operator' => '=', 'value' => $user_id]
        ]]);
        ...
        if ($update > 0) {
            ...
            $api_status_code_class_call->respondOK(
                ["user_id" => $user_id, "old_status" => $old_status, "new_status" => "suspended"],
                API_User_Response::$usersuspended
            );
        } else {
            $api_status_code_class_call->respondInternalError($update, API_User_Response::$failtosuspenduser);
        }
