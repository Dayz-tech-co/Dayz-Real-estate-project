...
try {
    ...
    $adminPubkey = isset($decodedToken->usertoken) ? $utility_class_call->clean_user_data($decodedToken->usertoken, 1) : '';
    $jti = isset($decodedToken->jti) ? $utility_class_call->clean_user_data($decodedToken->jti, 1) : '';

    if ($utility_class_call->input_is_invalid($adminPubkey) || $utility_class_call->input_is_invalid($jti)) {
        $api_status_call->respondUnauthorized();
    }

    $revoked = $api_status_call->revokeAuthSessionByJti($jti, $adminPubkey);
    if (!$revoked) {
