...
try {
    ...
    if ($utility_class_call->input_is_invalid($userPubkey) || $utility_class_call->input_is_invalid($jti)) {
        $api_status_call->respondUnauthorized();
    }

    $revoked = $api_status_call->revokeAuthSessionByJti($jti, $userPubkey);
    if (!$revoked) {
