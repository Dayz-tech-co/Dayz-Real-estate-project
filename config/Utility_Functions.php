...
class Utility_Functions  extends DB_Connect
{
    ...
    public static $bookingStatus = ['pending','awaiting_admin_approval','approved','rejected','cancelled','completed'];
    public static $bookingTypes  = ['sale','rental'];
    public static $paymentStatus = ['unpaid','paid','refunded'];
    public static $visitStatus   = ['pending','confirmed','completed','no_show'];
    public static $bookingStatusTransitions = [
        'pending' => ['awaiting_admin_approval', 'approved', 'rejected', 'cancelled'],
        'awaiting_admin_approval' => ['approved', 'rejected', 'cancelled'],
        'approved' => ['cancelled', 'completed'],
        'cancelled' => [],
        'rejected' => [],
        'completed' => []
    ];
    public static $visitStatusTransitions = [
    ...
    /**
     * Normalize pagination inputs with sensible defaults.
     */
    public static function normalizePagination($page, $limit): array
    {
        $p = (int)($page ?? 1);
        $l = (int)($limit ?? 20);
        if ($p < 1) {
    ...
    /**
     * Validate that a value is within an allowed enum set (case-insensitive).
     */
     ...
     */
    public static function validateDateYmd(?string $value): bool
    {
        if (empty($value)) {
            return true;
        }
    ...
    /**
     * Ensure end date is on/after start date.
     */
    public static function ensureDateOrder(?string $start, ?string $end): bool
    {
    ...
    /**
     * Normalize numeric input to int or null.
     */
    public static function normalizeInt($value): ?int
    {
        if ($value === null || $value === '') {
            return null;
        }
    ...
    /**
     * Check allowed booking status transition.
     */
    public static function canTransitionBookingStatus(string $from, string $to): bool
    {
        $from = strtolower($from);
        $to   = strtolower($to);
        if (!in_array($from, self::$bookingStatus, true) || !in_array($to, self::$bookingStatus, true)) {
            return false;
        }
        if ($from === $to) {
            return true;
        }
        return in_array($to, self::$bookingStatusTransitions[$from] ?? [], true);
    }

    /**
     * Check allowed visit status transition.
     */
    public static function canTransitionVisitStatus(string $from, string $to): bool
    {
        $from = strtolower($from);
        $to   = strtolower($to);
        if (!in_array($from, self::$visitStatus, true) || !in_array($to, self::$visitStatus, true)) {
            return false;
        }
        if ($from === $to) {
            return true;
        }
        return in_array($to, self::$visitStatusTransitions[$from] ?? [], true);
    }
    ...
    public static function setTimeZoneForUser($timeZoneToUse = '')
    {
        if (strlen($timeZoneToUse) > 2) {
            ...
            date_default_timezone_set($_ENV['COMPANY_TIME_LOCATION_ZONE']);
        }
    }
    public static function pingUrl($url)
    {
        $ch = curl_init($url);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, false); // Do not wait for response
        curl_setopt($ch, CURLOPT_TIMEOUT, 1);            // Timeout set to 1 second
        curl_setopt($ch, CURLOPT_CONNECTTIMEOUT, 30);     // Connection timeout set to 1 second
        curl_setopt($ch, CURLOPT_NOBODY, true);          // Do not include the body
        curl_setopt($ch, CURLOPT_HEADER, false);         // Do not include headers in output
        curl_exec($ch);
        curl_close($ch);
    }
    ...
    {
        // Add http or https to the URL if it's missing
    ...
    {
        return uniqid(strtolower(self::removeWhitespace($_ENV["APP_NAME"])), true) . self::removeWhitespace($attach);
    }
    /**
     * Check if input is empty,valid and ok
     * response String
    ...
    public static function exceptionHandler($exception)
    {
        ...
        if ($_ENV['APP_DEBUG']) {
            ...
            //SEND TO ADMIN TG
            self::system_notify_crash_handler($message, "System");
    ...
    public static function formatLargeNumber($number)
    {
        if ($number >= 1000000) {
            ...
            $x_count_parts = floor(log($number, 1000)); // Determines the appropriate suffix
            $x_display = $number / pow(1000, $x_count_parts); // Scales the number down

            // Convert the number to a string without rounding
            $x_display = preg_replace('/\.?0+$/', '', (string) $x_display);
     ...
     * 
     */
    public static function sendTelegramResponse($chatid, $message_id, $message, $keyboard, $botkey, $markdown = "html")
    {
        ...
        if (self::input_is_invalid($keyboard)) {
            $parameters = array(
                'chat_id' => $chatid,
                'text' => $message,
        ...
        //             "id": 6247233590,
        //             "is_bot": true,
        //             "first_name": "Level3 bot",
        //             "username": "Cardify_Level3_bot"
        //         },
        //         "chat": {
        //             "id": -982449392,
        //             "title": "D. Cardify KYC Group",
        //             "type": "group",
        //             "all_members_are_administrators": true
        //         },
        //         "date": 1718313802,
        //         "text": "KYC Verification\n\nFull name: Babs Imran\nEmail: habnarmtech@gmail.com\nUsername: Hab\nID Type:  National ID Card",
        //         "entities": [
        //             {
        //                 "offset": 0,
        //                 "length": 16,
        ...
        //                     {
        //                         "text": "View Selfie",
        //                         "url": "https://app.cardify.co/assets/images/userselfie/cardify666b6340126c15.42078768Hab.png"
        //                     },
        //                     {
        //                         "text": "View Gov ID",
        //                         "url": "https://app.cardify.co/assets/images/userregulatorycards/cardify666b63402b3734.00294317Hab.png"
        //                     },
        //                     {
        //                         "text": "View Utility Bill",
        //                         "url": "https://app.cardify.co/assets/images/userutility_bill/cardify666b6340b18275.62129589Hab.png"
        //                     }
        //                 ],
        //                 [
        //                     {
    ...
    public static function isImageTooLarge($file, $isMultiple = 0)
    {
        $imgSize = $file['size'];
        $error = $file['error'];
        $maxByte = $_ENV['MAX_FILE_UPLOAD'] * 1024 * 1024;
    ...
    {
        $botidtouse = $chatId = 0;
        $chatIdData = DB_Calls_Functions::selectRows("telegram_bots", 'groupid,botid',        [
            [
                ['column' => 'forwhat', 'operator' => '=', 'value' => 1],
            ]
        ]);
        if (!self::input_is_invalid($chatIdData)) {
            $chatId = $chatIdData[0]['groupid'];
            $botidtouse = $chatIdData[0]['botid'];
        }
        ...
        self::sendTelegramResponse($chatId, "0", $response, $keyboard, $botidtouse, "markdown");
    }

    public static function send_message_to_admin_tg($message, $title, $whichbot = 2)
    {
        $botidtouse = $chatId = 0;
        $chatIdData = DB_Calls_Functions::selectRows("telegram_bots", 'groupid,botid',        [
            [
                ['column' => 'forwhat', 'operator' => '=', 'value' => $whichbot],
            ]
        ]);
        if (!self::input_is_invalid($chatIdData)) {
            $chatId = $chatIdData[0]['groupid'];
    ...
    public static function assignTask($title, $description, $assignedById, $assignedByRole, $assignedToId, $assignedToRole, $priority = 'medium', $dueDate = null)
    {
        $connect = static::getDB();
        // Clean inputs
        $title = static::clean_user_data($title);
        $description = static::clean_user_data($description, 1); // allow special chars
        $priority = static::clean_user_data($priority);
        $assignedByRole = strtolower(static::clean_user_data($assignedByRole));
        $assignedToRole = strtolower(static::clean_user_data($assignedToRole));
        $status = 'pending';

        // Validation: Users cannot assign tasks to admins
        if ($assignedByRole === "user" && $assignedToRole === "admin") {
            return [
                "status" => false,
                "message" => "Users are not allowed to assign tasks to admins"
            ];
        }

        // Build task data array
        $insertData = [
            "title" => $title,
        ...
        // 📥 Insert into DB
        $taskId = \Config\DB_Calls_Functions::insertRow("tasks", $insertData);
    ...
    public static function doesTaskExist($title)
    {
        $connect = static::getDB();

        $title = static::clean_user_data($title);

        $task =  DB_Calls_Functions::selectRows("tasks", "*", [
            [
                ['column' => 'title', 'operator' => '=', 'value' => $title]
            ]
        ]);

        return !static::input_is_invalid($task);
    }

    public function log_to_file($filename, $data)
    {
        $filePath = __DIR__ . "/../../logs/" . $filename; // adjust path if needed
