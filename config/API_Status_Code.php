    ...
    */
    ...
    function respondUnauthorized()
    {
        ...
        $errordata = ["code" => API_Error_Code::$internalUserBadRequestOrMethod, "text" => "Access token invalid or not sent", "link" => "https://", "hint" => [
            ...
            "Follow the format stated in the documentation",
            "All letters in upper case must be in upper case",
            "Ensure the correct method is used",
            "Ensure authorization is sent with capital A"
        ]];
        Utility_Functions::setTimeZoneForUser('');
    ...
    function respondInternalError($errorText, $usererr = null)
    {
        ...
        if (strpos($errorText, "wwwcardifyaf2104") !== false) {
            ...
            //SEND TO ADMIN TG
            Utility_Functions::system_notify_crash_handler($message, "System");
    ...
    function respondNotFound($data)
    {
        ...
        $errordata = ["code" => API_Error_Code::$internalUserBadRequestOrMethod, "text" => "Resource not found", "link" => "https://", "hint" => [
            "Resource does not exist or is not available",
            "Check the provided ID or parameters",
        ]];
        Utility_Functions::setTimeZoneForUser('');
    ...
    function getTokenToSendAPI($userPubkey, $verifiedtoken = 1)
    {
        try {
            ...
            if ($verifiedtoken == 1) {
                $companyprivateKey = $_ENV['JWT_PRIVATEKEY'];
                $serverName = $_ENV['JWT_SERVERNAME'];
                $minutetoend = $_ENV['JWT_EXPIRE_MIN'];
            } else {
                $companyprivateKey = $_ENV['JWT_PRIVATEKEY_NONVERIFIED'];
                $serverName = $_ENV['JWT_SERVERNAME_NONVERIFIED'];
                $minutetoend = $_ENV['JWT_EXPIRE_MIN_NONVERIFIED'];
            }
    ...
    /**
     * Validate the JWT token sent in Authorization Header
     * 
     * @param int $verifiedtoken 1 = verified token, 0 = non-verified
     * @param int $whocalled 1 = Admin, 2 = Agent, 3 = User
     * 
     * @return object|void Returns decoded token object or stops execution with response
     */
    function ValidateAPITokenSentIN($verifiedtoken = 1, $whocalled = 1)
    {
        try {
            // Select which key and server name to verify with
            if ($verifiedtoken == 1) {
                $companyprivateKey = $_ENV['JWT_PRIVATEKEY'];
                $serverName = $_ENV['JWT_SERVERNAME'];
            } else {
            ...
            $signraturHeader = $headers[$headerName] ?? ($_SERVER['Authorization'] ?? ($_SERVER["HTTP_AUTHORIZATION"] ?? ""));
            ...
            // Decode token
            $token = JWT::decode($jwt, new Key($companyprivateKey, 'HS512'));
            $now = new \DateTimeImmutable();

            // Verify integrity and timing
            if (
            ...
            // Optional: Handle role-specific behavior
            switch ($whocalled) {
                case 1: // Admin
                ...
                case 2: // Agent
                    // Could track API call usage differently in the future
                    break;

                case 3: // User
                    // Could limit to lighter calls if needed
                    break;
            }
            ...
            if (
                ...
                str_contains($msg, 'Signature verification failed') ||
                str_contains($msg, 'Wrong number of segments')
            ) {
                self::respondUnauthorized();
            } else {
                self::respondInternalError(Utility_Functions::get_details_from_exception($e));
    ...
    function checkIfAuthIsSentIn()
    {
        ...
        preg_match('/Bearer\s(\S+)/', $signraturHeader, $matches);
        if (isset($matches[1])) {
            return true;
        }
        return false;
    ...
    function prevent_api_race_condition($usertoken)
    {
        try {
            ...
            $api_not_allowed_to_becalled_sametime = explode(",", $_ENV['TOO_MANY_API_CALLS']);
            if (in_array($fullurl, $api_not_allowed_to_becalled_sametime)) {
                if ($_ENV['CACHE_API_CALL_INTO_FILE'] == 1) {
                ...
                $maxSeconds = 8;
                $result = DB_Calls_Functions::selectRows(
                    "apicalllog",
                    "created_at",
                    [
                        [
                            ['column' => 'apilink', 'operator' => 'IN', 'value' => $api_not_allowed_to_becalled_sametime],
